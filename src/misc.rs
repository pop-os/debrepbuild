...
pub fn remove_empty_directories_from(directory: &Path) -> io::Result<bool> {
    let mut empty = true;
    let entries = directory.read_dir().map_err(|why| {
        Error::new(
            ErrorKind::Other,
    ...
    if empty {
        log::info!("removing {} because it is empty", directory.display());
        fs::remove_dir(directory).map_err(|why| {
            Error::new(
                ErrorKind::Other,
