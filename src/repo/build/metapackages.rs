use super::super::pool::{ARCHIVES_ONLY, mv_to_pool};
...
pub fn generate(suite: &str, component: &str) -> io::Result<()> {
    ...
    log::info!("removing any leftover deb archives in the metapackages directory");
    for entry in metapackages.read_dir()? {
        let entry = entry?;
        if entry
            .file_name()
            .to_str()
            .map_or(false, |e| e.ends_with(".deb"))
        {
            fs::remove_file(entry.path())?;
        }
    }

    log::info!("generating metapackages");
...
fn is_cfg(entry: &DirEntry) -> bool {
    !entry.path().is_dir()
        && entry
            .file_name()
            .to_str()
            .map_or(false, |e| e.ends_with(".cfg"))
}
...
fn inner_generate(entry: &DirEntry) -> io::Result<()> {
    ...
    log::info!("generating metapackage at {}", path.display());
    ...
    directory_scope(parent, move || {
        Command::new("equivs-build").arg(filename).run()
    })
}
