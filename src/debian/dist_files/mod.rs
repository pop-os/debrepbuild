            ...
            .map(|(arch, (packages, contents))| {
                ...
                let (contents_res, packages_res) = rayon::join(
                    ...
                    || {
                        ...
                        let mut contents =
                            ContentsIterator::new(contents).collect::<Vec<Vec<u8>>>();
                            ...
                            .map(|(component, mut packages)| {
                                ...
                                .map_err(|why| {
                                    io::Error::new(
                                        io::ErrorKind::Other,
                                        format!(
                                            "failed to generate content archive at {}: {}",
                                            path.display(),
                                            why
                                        ),
                                    )
                                ...
                                inner_write_release_file(config, binary_path, arch_dir, &component)
                                    .map_err(|why| {
                                        io::Error::new(
                                            io::ErrorKind::Other,
                                            format!(
                                                "failed to create release file for {}: {}",
                                                binary_path.display(),
                                                why
                                            ),
                                        )
        ...
        loop {
            ...
            match contents_entry.files.get(self.file) {
                Some(path) => {
                    ...
                    self.buffer.extend_from_slice(if &path[..2] == b"./" {
                        &path[2..]
                    } else {
                        path
                    });
                    ...
                    self.buffer
                        .extend_from_slice(contents_entry.package.as_bytes());
                    self.buffer.push(b'\n');
